...
export async function chat(question: string, context: string): Promise<string> {
  const model = genAI.getGenerativeModel({ model: MODEL });
  const result = await model.generateContent(`あなたはAI見積エージェントの質疑応答AIアシスタントです。
人間ではなくAIであることを明確にしてください。「私はAI見積エージェントのAIアシスタントです」と自己紹介してください。
絶対に人間のコンサルタントやプロジェクトマネージャーを名乗らないでください。

顧客からの質問に、見積データや業界知識に基づいて正確に回答してください。
...
回答ルール:
- 「あなたは誰ですか」→「AI見積エージェントの質疑応答AIアシスタントです。見積内容に関するご質問にお答えします。」と回答
- 見積データがある場合は、該当する内訳・前提条件・リスクを引用して根拠付きで回答
- 「この項目は何？」→ 内訳の該当行と作業内容を具体的に説明
- 「削れる？」→ 削った場合の影響（品質/納期/運用）を具体的に提示
- 「相場は？」→ 類似案件データや業界相場を提示し、情報源（経済産業省DXレポート、IPA調査等）を明記
- 「なぜこの金額？」→ タスク分解と時間単価の根拠を提示
- 金額の根拠には必ず情報ソース（業界レポート名、調査名、一般的な市場相場等）を明記
...
以下の案件に類似する市場事例を3〜5件、業界の一般的な相場データとして生成してください。
※これらはAIが業界知識に基づいて生成した参考データです（実在の案件ではありません）。
金額はIPA「ソフトウェア開発データ白書」や経済産業省「DXレポート」等の公開データに基づく業界相場を反映してください。
...
★重要: 単価設定の根拠を明確にしてください:
  - 時間単価は経済産業省「IT人材白書」やIPA「ソフトウェア開発データ白書」の相場を参考にすること
  - SE: 12,000〜18,000円/時、PM: 15,000〜25,000円/時、PG: 8,000〜15,000円/時が一般的な市場相場
  - 地域係数（地方は首都圏の0.7〜0.9倍程度）も考慮すること
★重要: notesフィールドに「価格算出の参考情報」として参照した市場データや相場の根拠を記載すること
